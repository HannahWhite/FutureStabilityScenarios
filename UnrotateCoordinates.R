#####################################################
### Convert rotated lat-long to standard lat-long ###
#####################################################
...
lon <- ncvar_get(pr.40, "rlon")
lat <- ncvar_get(pr.40, "rlat", verbose = F) 
day <- ncvar_get(pr.40, "time") # in days since 1949-12-01
